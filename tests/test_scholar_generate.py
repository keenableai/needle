...
async def test_arxiv_candidates_vary_with_seed():
    papers = [_paper(i, "computer science", datetime(2026, 7, 1, tzinfo=UTC)) for i in range(30)]
    bodies = {p.arxiv_id: f"body {p.arxiv_id}" for p in papers}
    llm = FakeLLM({p.arxiv_id: f"distinct anchor beta {p.arxiv_id}" for p in papers})
    picks = []
    for seed in (0, 1):
        ...
        picks.append({r["gold"]["paper_key"] for r in rows})
    assert picks[0] and picks[1]
